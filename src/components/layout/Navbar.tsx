...
export const Navbar = () => {
  ...
  const getDashboardLink = () => {
    ...
    if (roleStr === 'employer') return '/employer/dashboard';
    if (roleStr === 'job_seeker' || roleStr === 'jobseeker') return '/jobseeker/dashboard';
    return null;
  };
  ...
  const getRoleLinks = () => {
    ...
    if (roleStr === 'job_seeker' || roleStr === 'jobseeker') {
      return (
        <>
          <Link 
            to="/jobs"
          ...
          <Link 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              isActiveLink('/jobseeker/applications')
                ? 'bg-indigo-50 text-indigo-600'
                : 'text-gray-700 hover:bg-gray-50 hover:text-indigo-600'
            }`}
          >
