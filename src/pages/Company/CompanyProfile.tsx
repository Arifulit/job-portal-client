...
const CompanyProfile: React.FC = () => {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useCompanyProfile(id, page, limit, reviewsLimit);
  ...
  useEffect(() => {
    if (isError && retryCount < 5) {
      // Show error message only after 3 retries
      if (retryCount >= 2) {
        setShowError(true);
      }
      ...
      const timer = setTimeout(() => {
        ...
        refetch();
      }, 2000 + retryCount * 1000); // Progressive delay: 2s, 3s, 4s, 5s, 6s
      
      return () => clearTimeout(timer);
    } else if (isError && retryCount >= 5) {
      setShowError(true);
    }
    ...
    return (
      <div className="min-h-screen bg-slate-50 px-4 py-8">
        <div className="mx-auto max-w-xl rounded-2xl border border-red-200 bg-red-50 p-6 text-center">
          ...
          <p className="mt-2 text-sm text-red-600">Attempted {retryCount} retries. Please try again or contact support.</p>
          <button
            type="button"
            onClick={() => {
              setRetryCount(0);
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_360px]">
          ...
          <aside className="space-y-6">
            ...
            {/* Write a Review Section - Only visible to candidates */}
            {isCandidate && (
              <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="flex items-center justify-between gap-2">
